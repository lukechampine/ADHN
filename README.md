...
ADHN ("Attention-Deficient Hacker News") is a simple userscript that adds inline article summaries to Hacker News. It uses the Clipped API, mostly because that was the only summarizing tool I could find with an API that wasn't rate limited. In my experience, the summaries tend to resemble the first few lines of the article, which works great in some cases and abysmally in others.
